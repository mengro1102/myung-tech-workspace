...
| YouTube 콘텐츠 조회 | 영상 링크 · 채널 링크 · `@핸들` · "유튜브에서 ~ 찾아/검색" |
...
<a id="youtube-lookup"></a>
## 🔎 YouTube 콘텐츠 조회 — 남의 영상·채널·검색

**추가로 넣을 값 없음.** 📺 YouTube Data API 의 API 키를 그대로 쓴다.
(자막 언어 목록만 📊 OAuth 연결이 있어야 나온다.)

**이렇게 쓴다** — 지시문에 아래 중 하나가 있으면 서버가 먼저 조회해서 에이전트에게 근거로 넘긴다.

| 지시문에 넣은 것 | 가져오는 것 |
|---|---|
| 영상 링크 (`youtube.com/watch?v=…`, `youtu.be/…`, shorts, live) | 제목 · 채널 · 길이 · 조회/좋아요/댓글 · 태그 · 설명 · 챕터 · **자막 언어** · 인기 댓글 5개 |
| 채널 링크 또는 `@핸들` | 구독자 · 영상 수 · 총 조회 · 소개 · 최근 영상 6개 |
| "유튜브에서 ○○ 찾아줘" | 검색 상위 6개 영상과 각각의 지표 |

예) `이 영상 구성 분석해서 우리 채널용 기획안 짜줘 https://youtu.be/…`
예) `@채널핸들 은 어떤 주제로 영상을 올리는지 정리해줘`
예) `유튜브에서 RAG 튜토리얼 영상 찾아서 비교해줘`

**자막 원문은 가져올 수 없다.** YouTube API 문서상 자막 다운로드는 **그 영상의 편집 권한이 있는
사람만** 할 수 있다. 남의 자막을 긁어 오는 비공식 방법은 YouTube 약관의 스크래핑 금지에 걸리므로
넣지 않았다. 대신 어떤 언어 자막이 있는지, 설명과 챕터, 인기 댓글을 넣는다 — 영상의 뼈대는 대개
거기서 드러난다. 내 영상의 자막은 받을 수 있다(필요하면 말해 달라).

**남의 콘텐츠를 물으면 내 채널 지표는 붙이지 않는다.** 둘이 같이 들어가면 모델이 그 영상을 내
것으로 섞어 읽는다. 내 채널과 비교하고 싶으면 지시문에 "내 채널" 이라고 쓰면 둘 다 붙는다.

**할당량** — 하루 10,000 단위 중 영상·채널 조회 1, 댓글 1, 자막 목록 50, **검색 100**. 검색은
하루 약 100번이 한도라, 지시문이 분명히 검색을 요구할 때만 하고 한 번에 한 번만 한다.
