...
powershell -NoProfile -Command "try{$h=Invoke-RestMethod 'http://localhost:9000/api/health' -TimeoutSec 10; ('        server {0} / AI model {1} / KB {2}' -f $h.server.ok, $h.vllm.ok, $h.knowledge_base.ok)}catch{'        API no response'}" 2>nul
